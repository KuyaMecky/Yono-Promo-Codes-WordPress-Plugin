 ...
 * Description: Promo codes with scheduling + floating widget & modal, and a Games grid with upcoming countdown, search, filter, sort, CSV import/export. Includes Media Library picker for game logos (with URL fallback) + WIDE responsive game card layout.
 * Version: 2.3.0
...
class Yono_Promos {
  ...
  public function admin_assets($hook){
    ...
    if (($hook==='post-new.php'||$hook==='post.php') && $post_type===self::CPT){
      wp_enqueue_style('yono-promos-admin', plugin_dir_url(__FILE__).'assets/css/promos.css',[], '2.3.0');
      wp_enqueue_script('yono-promos-admin', plugin_dir_url(__FILE__).'assets/js/promos.js',['jquery'],'2.3.0', true);
    }
  }

  public function enqueue_assets(){
    // front-end assets (your existing files)
    wp_enqueue_style('yono-promos', plugin_dir_url(__FILE__).'assets/css/promos.css',[], '2.3.0');
    wp_enqueue_script('yono-promos', plugin_dir_url(__FILE__).'assets/js/promos.js',[], '2.3.0', true);
  }
  ...
  public function render_floating_widget(){
    ...
    // show widget only if there is at least one active or upcoming promo
    $q = new WP_Query([
      'post_type' => self::CPT, 'posts_per_page' => 1,
    ...
    $settings_json = esc_attr( wp_json_encode([
      'now'=>gmdate('c'), 'showCopy'=>true, 'showTimer'=>true, 'format'=>'long'
    ]) );
    ...
    (function(){
      ...
      document.addEventListener('DOMContentLoaded', function(){
        var popup=document.getElementById('promoPopup'); var trigger=document.querySelector('.promo-trigger');
        ...
        function activate(period){
          tabs.forEach(function(t){var on=t.getAttribute('data-period')===period; t.classList.toggle('active',on); t.setAttribute('aria-selected',on?'true':'false');});
          panes.forEach(function(p){p.classList.toggle('active',p.getAttribute('data-period')===period);});
        }
        function openP(){ popup.hidden=false; popup.classList.add('show'); document.documentElement.style.overflow='hidden'; trigger.setAttribute('aria-expanded','true'); activate('morning'); }
        function closeP(){ popup.classList.remove('show'); popup.hidden=true; document.documentElement.style.overflow=''; trigger.setAttribute('aria-expanded','false'); }
        ...
        popup.addEventListener('click', function(e){
          var btn=e.target.closest && e.target.closest('.promo-copy'); if(!btn||!toast) return;
          toast.classList.add('show'); setTimeout(function(){ toast.classList.remove('show'); },1200);
        });
      });
    })();
    </script>
...
/* =========================================================
 * GAMES + CSV + MEDIA PICKER (+ WIDE RESPONSIVE LAYOUT)
 * ========================================================= */
class Yono_Games {
  const CPT  = 'yono_game';
  ...
  public function enqueue_assets(){
    wp_enqueue_style('yono-games', plugin_dir_url(__FILE__).'assets/css/games.css',[], '2.3.0');
    wp_enqueue_script('yono-games', plugin_dir_url(__FILE__).'assets/js/games.js',[], '2.3.0', true);
  }

  public function admin_assets($hook){
    if (strpos($hook,'yono_games_tools') !== false){
      wp_enqueue_style('yono-games', plugin_dir_url(__FILE__).'assets/css/games.css',[], '2.3.0');
      wp_enqueue_script('yono-games-admin', plugin_dir_url(__FILE__).'assets/js/games-admin.js',[], '2.3.0', true);
    }
    global $post_type;
    if (($hook==='post-new.php' || $hook==='post.php') && $post_type===self::CPT){
      wp_enqueue_media();
      wp_enqueue_script('yono-media-picker', plugin_dir_url(__FILE__).'assets/js/yono-media-picker.js',['jquery'],'2.3.0', true);
    }
  }
  ...
  public function render_meta_box($post){
    ...
    <div class="yono-grid-2">
      ...
      <!---->
      <p><label><strong>Download URL</strong></label><br>
  <input type="url" name="yg_download_url" class="widefat"
         placeholder="https://example.com/app.apk"
         value="<?php echo esc_attr( get_post_meta($post->ID,'_yg_download_url',true) ); ?>">
...
<!---->
    ...
    <p>
      ...
      <textarea name="yg_desc" class="widefat" rows="4" placeholder="Short description shown in the wide card"><?php echo esc_textarea($desc); ?></textarea>
    </p>
...
public function shortcode($atts){
  $atts = shortcode_atts([
    ...
    'badge'       => '',
    'per_page'    => '60',
    'columns'     => '3',          // 1..6 controls density on large screens
    'sort'        => 'name',
    'show_count'  => 'true',
    'show_search' => 'true',
    'show_filters'=> 'true',
  ...
  while($q->have_posts()){ $q->the_post();
    ...
    $items[] = [
      ...
      'launch'=>$launch,'status'=>$status,'cat'=>$cats,'badge'=>$badgs
    ];
  } wp_reset_postdata();
  ...
  // Map columns → minimum card width for auto-fit grid.
  $cols = max(1, min(6, intval($atts['columns'])));
  $min_by_cols = [
    1 => '100%',   // force one per row on large screens
    2 => '520px',
    3 => '360px',
    4 => '280px',
    5 => '220px',
  ...
  // Inject base styles once to avoid "unstyled" toolbar / inputs
  static $once = false;
  if (!$once){
    $once = true;
    echo '<style id="yono-games-inline-base">
      .yono-games{--card-min:280px}
      .yono-games .yg-toolbar{display:grid;gap:12px;margin-bottom:16px;    justify-items: center;}
      .yono-games .yg-search{
        width:100%; padding:12px 14px; border-radius:10px; border:1px solid rgba(148,163,184,.24);
        background:#0b1220; color:#e8f0fb; font-size:14px; outline:none;
      }
      .yono-games .yg-count{color:#cbd5e1; font-weight:600}
      .yono-games .yg-cat-chips{display:flex;flex-wrap:wrap;gap:8px}
      .yono-games .yg-chip{
        background:#102138; color:#cfe1ff; border:1px solid rgba(57,78,140,.55);
        padding:6px 10px; border-radius:999px; font-weight:800; font-size:12px; cursor:pointer;
      }
      .yono-games .yg-chip.is-active{background:#1e293b;color:#fff;border-color:#3b82f6}
      .yono-games .yg-sort{display:flex;align-items:center;gap:8px;color:#9aa4b2;font-weight:700}
      .yono-games .yg-sort-select{
        appearance:none; width:100%; max-width:280px; padding:10px 12px; border-radius:10px;
        border:1px solid rgba(148,163,184,.24); background:#0b1220; color:#e8f0fb;
      }
      /* Compact card grid – responsive columns via auto-fit + min width */
      .yono-games:not(.yono-games--wide) .yg-grid{
        display:grid; gap:16px;
        grid-template-columns: repeat(auto-fit, minmax(var(--card-min,280px), 1fr));
      }
      /* Wide card grid – one per row, still responsive */
      /* The min width (minmax) keeps the nice big card size you like */
        .yono-games.yono-games--wide .yg-grid{
          display:grid;
          gap:18px;
          grid-template-columns:repeat(var(--wide-cols,1), minmax(680px, 1fr));
        }
  ...
  // Inline extra CSS for wide layout visuals (already present in your plugin – kept as-is)
  if ($is_wide){
    echo '<style>
      .yg-card--wide{display:grid;grid-template-columns:minmax(220px,32vw) 1fr;gap:24px;align-items:start;
        background:#0b1220;border:1px solid rgba(148,163,184,.14);border-radius:18px;padding:18px;
        box-shadow:0 10px 28px rgba(0,0,0,.25)}
      @media (min-width:1100px){ .yg-card--wide{ grid-template-columns:300px 1fr } }
      @media (max-width:860px){ .yg-card--wide{ grid-template-columns:1fr; } }
      .yg-wide-left{position:relative}
      .yg-wide-logo{width:100%;aspect-ratio:1/1;border-radius:14px;object-fit:cover;background:#0e1726;border:1px solid rgba(148,163,184,.14)}
      .yg-wide-logo--placeholder{width:100%;aspect-ratio:1/1;border-radius:14px;display:grid;place-items:center;background:#2f3545;color:#cbd5e1;font-weight:700;border:1px dashed rgba(148,163,184,.3)}
      .yg-wide-right{min-width:0;display:grid;grid-template-rows:auto auto auto auto auto;row-gap:12px}
      .yg-wide-badges{display:flex;gap:8px;align-items:center;flex-wrap:wrap}
      .yg-badge{background:#2a1d00;color:#fbbf24;border:1px solid #f59e0b;padding:4px 8px;border-radius:8px;font-size:11px;font-weight:800}
      ...
      .yg-wide-title{margin:0;font-size:clamp(22px,2.4vw,30px);line-height:1.15;color:#e8f0fb;font-weight:800}
      .yg-wide-sub{color:#9aa4b2;font-size:13px}
      .yg-wide-desc{background:#0e1726;border:1px solid rgba(148,163,184,.14);border-radius:12px;padding:12px 14px;color:#cbd5e1;font-size:14px}
      .yg-wide-desc>div+div{margin-top:6px}
      .yg-wide-meta{display:flex;gap:14px;align-items:center;flex-wrap:wrap}
      .yg-chip{background:#1e293b;color:#cbd5e1;border:1px solid rgba(148,163,184,.25);padding:6px 10px;border-radius:10px;font-weight:700;font-size:12px}
      .yg-wide-date{color:#9aa4b2;font-size:12.5px}
      .yg-wide-cta-row{display:flex;align-items:center;gap:12px;margin-top:2px}
      .yg-cta--wide{flex:1 1 auto;display:inline-flex;justify-content:center;align-items:center;padding:14px 18px;border-radius:12px;border:0;
        background:linear-gradient(135deg,#f97316,#ef4444);color:#fff;font-weight:800;text-decoration:none}
      .yg-cta--wide[disabled]{opacity:.6;cursor:not-allowed}
      .yg-save--wide{background:#142235;color:#cbd5e1;border:1px solid rgba(148,163,184,.25);border-radius:12px;padding:12px 14px;cursor:pointer}
      .yg-card--wide.is-upcoming{border-color:rgba(59,130,246,.35)}
    </style>';
  }
...
<section class="yono-games<?php echo $is_wide ? ' yono-games--wide' : ''; ?>"
        ...
        <?php else: ?>
          <article class="yg-card yg-card--wide <?php echo $is_up ? 'is-upcoming':''; ?>"
            ...
            <div class="yg-wide-left">
              ...
              <?php else: ?>
                <div class="yg-wide-logo--placeholder">LOGO</div>
              <?php endif; ?>
            </div>
            <div class="yg-wide-right">
              <div class="yg-wide-badges">
                <?php if (!empty($g['badge'])) foreach($g['badge'] as $b): ?>
                  <span class="yg-badge"><?php echo esc_html(ucwords(str_replace('-',' ',$b))); ?></span>
                <?php endforeach; ?>
                <?php if (!empty($g['cat'])) foreach($g['cat'] as $c): ?>
                  <span class="yg-pill"><?php echo esc_html($c); ?></span>
                <?php endforeach; ?>
              </div>
              <h2 class="yg-wide-title"><?php echo esc_html($g['title']); ?></h2>
              <?php if ($g['subtitle']): ?><div class="yg-wide-sub"><?php echo esc_html($g['subtitle']); ?></div><?php endif; ?>
              ...
              <div class="yg-wide-meta">
                <?php if ($is_up): ?>
                  <span class="yg-chip">Launches in: <span class="yg-countdown" aria-live="polite">—</span></span>
                <?php endif; ?>
                <?php if (!empty($g['launch'])): ?>
                  <span class="yg-wide-date">Date: <?php echo esc_html( get_date_from_gmt( gmdate('Y-m-d H:i:s', strtotime($g['launch'])), 'M j, Y H:i' ) ); ?></span>
                <?php endif; ?>
              </div>
              <div class="yg-wide-cta-row">
                <?php if ($g['cta_url']): ?>
                  <a class="yg-cta yg-cta--wide" href="<?php echo esc_url($g['cta_url']); ?>" target="_blank" rel="nofollow noopener"><?php echo esc_html($g['cta_text']); ?></a>
                <?php else: ?>
  ...
  public function render_tools_page(){ ?>
    <div class="wrap">
      ...
      <p>CSV headers: <code>title,subtitle,category,badge,logo,bonus_range,min_withdraw,cta_text,cta_url,launch_at,status,description</code></p>
      <ol>
  ...
  public function handle_export(){
    ...
    fputcsv($out, ['title','subtitle','category','badge','logo','bonus_range','min_withdraw','cta_text','cta_url','launch_at','status','description']);
    while($q->have_posts()){ $q->the_post();
      $id=get_the_ID();
      $cats = wp_get_post_terms($id,self::TAX_CAT,['fields'=>'names']);
  ...
  public function handle_import(){
    ...
    while(($row=fgetcsv($fh))!==false){
      ...
      update_post_meta($post_id,'_yg_desc',         sanitize_textarea_field($row[$map['description']] ?? ''));
